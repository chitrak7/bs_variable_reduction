...
OLSE <- function(X, y,d,m,mu,n) {
  ...
  X <- X
  y <- y - rep(mu,n)/d
  res <- cv.glmnet(X, y, family="gaussian",intercept=(1==0),alpha=alpha,weights = d)
  lambda_min <- res$lambda.min
  res <- glmnet(X, y, family = "gaussian",intercept=(1==0), lambda = lambda_min, alpha=alpha, weights = d)
  beta <- coef(res)
...
##############################
# st <- Sys.time()
# result <- matrix(nrow=200, ncol=1000000)
# for(i in 1:200){result[i,] <- simplef(1000000)}
# print(Sys.time()-st)
# 
# nc <- detectCores()-1
# cl <- makeCluster(nc)
...
# result1 <- foreach(i=1:200) %dopar% simplef(1000000)
# print(Sys.time()-st)
# stopCluster(cl)
###################################
